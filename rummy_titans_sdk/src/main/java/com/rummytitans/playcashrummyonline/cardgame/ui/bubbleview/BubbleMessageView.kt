...
class BubbleMessageView : ConstraintLayout {
    ...
    private fun inflateXML() {
        itemView = inflate(context, R.layout.view_bubble_message_rummy, this)
    }

    private fun bindViews() {
        imageViewIcon = findViewById(R.id.imageViewShowCase)
        imageViewClose = findViewById(R.id.Coach_Cancel)
    ...
    private fun setAttributes(builder: Builder) {
        ...
        imageViewClose?.visibility=if (builder.mDisableCloseAction!=null && builder.mDisableCloseAction==true) {
              View.INVISIBLE
        }else
            View.VISIBLE
    ...
    private fun getArrowHorizontalPositionDependingOnTarget(targetViewLocationOnScreen: RectF?): Int {
        ...
        when {
            ...
            isOutOfLeftBound(targetViewLocationOnScreen) -> xPosition = getSecurityArrowMargin()
                ...
                Math.round(targetViewLocationOnScreen!!.centerX() - ScreenUtils.getAxisXpositionOfViewOnScreen(this))
        }
        return xPosition
    }
    ...
    private fun isOutOfBottomBound(targetViewLocationOnScreen: RectF?): Boolean {
        return targetViewLocationOnScreen!!.centerY() > ScreenUtils.getAxisYpositionOfViewOnScreen(this) + height - getSecurityArrowMargin() - ScreenUtils.getStatusBarHeight(
            context
        )
    }

    private fun isOutOfTopBound(targetViewLocationOnScreen: RectF?): Boolean {
        return targetViewLocationOnScreen!!.centerY() < ScreenUtils.getAxisYpositionOfViewOnScreen(this) + getSecurityArrowMargin() - ScreenUtils.getStatusBarHeight(
            context
        )
    }
