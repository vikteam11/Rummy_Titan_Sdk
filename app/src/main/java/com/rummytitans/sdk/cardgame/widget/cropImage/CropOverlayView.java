...
package com.rummytitans.sdk.cardgame.widget.cropImage;
...
public class CropOverlayView extends View {
  ...
  private void initCropWindow() {
    ...
    if (mInitialCropWindowRect.width() > 0 && mInitialCropWindowRect.height() > 0) {
      ...
      rect.left = leftLimit + 10 ; // horizontalPadding;
      rect.top = topLimit +10; //verticalPadding;
      rect.right = rightLimit - 10 ;//;horizontalPadding;
      rect.bottom = bottomLimit - 10;//verticalPadding;
      int widthR = (int) rect.width();
      int heightR = (int) rect.height();
      int dif = heightR-widthR;
      if(heightR > widthR && dif >100){
         rect.top = (float) (topLimit + (heightR * .30));
         rect.bottom = (float) (bottomLimit - (heightR * .30));
      }
    }
